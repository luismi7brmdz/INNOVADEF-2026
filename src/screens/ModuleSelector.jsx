...
export default function ModuleSelector({ onSelect, bootStage = 4 }) {
  ...
  return (
    <div style={{ width: '100%', margin: '0 auto' }}>
      ...
      {/* System info panels */}
      <div className="info-panels-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 'clamp(10px, 1.5vw, 16px)', marginBottom: 'clamp(16px, 3vw, 36px)' }}>
        <style>{`
        ...
        {[
          {
            label: '// SISTEMA',
            content: (
              <div style={{ padding: 'clamp(10px, 1.5vw, 16px) clamp(12px, 2vw, 20px)', fontFamily: FONT, fontSize: 'clamp(12px, 1.2vw, 14px)', lineHeight: 1.8, color: TEXT2 }}>
          ...
          },
          {
            label: '// SISTEMAS',
            content: (
              <div style={{ padding: 'clamp(10px, 1.5vw, 16px) clamp(12px, 2vw, 20px)', fontFamily: FONT, fontSize: 'clamp(12px, 1.2vw, 14px)', lineHeight: 1.8 }}>
                {[
      ...
      {/* Title */}
      <div style={{
        marginBottom: 'clamp(14px, 3vw, 40px)',
        borderBottom: `1.5px solid ${BORDER}`,
        paddingBottom: 'clamp(10px, 2vw, 22px)',
        ...vis(3, 540),
      }}>
        <div className="selector-subtitle" style={{ fontFamily: FONT, fontSize: 'clamp(12px, 1.8vw, 22px)', color: TEXT2, letterSpacing: 'clamp(1px, 0.4vw, 4px)', marginBottom: 'clamp(8px, 2vw, 28px)' }}>
          // SISTEMA DE EVALUACIÓN INTERACTIVA
        </div>
        <div style={{ fontFamily: FONT, fontSize: 'clamp(22px, 4.5vw, 72px)', letterSpacing: 'clamp(2px, 0.6vw, 6px)', color: ACCENT, textShadow: `0 0 60px ${ACCENT}33`, lineHeight: 1.15 }}>
          CENTRO DE OPERACIONES<br />
          <span style={{ color: TEXT2, fontSize: 'clamp(14px, 2.5vw, 48px)' }}>INNOVADEF FOCO 2026 — MADRID</span>
        </div>
      </div>
      ...
      {Object.entries(byCategory).map(([cat, mods], catIdx) => (
        <div key={cat} style={{ marginBottom: 'clamp(16px, 3vw, 40px)' }}>
          ...
          <div style={{
            ...
            <span style={{ fontFamily: FONT, fontSize: 'clamp(13px, 2vw, 19px)', color: TEXT2, letterSpacing: 'clamp(2px, 0.6vw, 5px)', textTransform: 'uppercase' }}>
              // {cat.toUpperCase()}
            </span>
            <div style={{ height: '1px', background: BORDER, flex: 1 }} />
          </div>
          ...
          {/* Cards grid */}
          <div className="module-cards-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, clamp(260px, 40vw, 520px)), 1fr))', gap: 'clamp(10px, 1.5vw, 20px)', alignItems: 'stretch' }}>
            {mods.map(mod => {
              const idx = globalIdx++
              const Icon = mod.icon
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      <div style={{ display: 'flex', gap: 'clamp(6px, 1vw, 12px)', alignItems: 'center' }}>
                        ...
                        <span style={{ fontFamily: FONT, fontSize: 'clamp(13px, 2vw, 22px)', color: TEXT2, letterSpacing: 'clamp(1px, 0.4vw, 3px)' }}>
                          {mod.code}
                        </span>
                      </div>
                      <div style={{ display: 'flex', gap: 'clamp(6px, 1vw, 14px)', alignItems: 'center' }}>
                        <span style={{
                          fontFamily: FONT, fontSize: 'clamp(11px, 1.5vw, 18px)', letterSpacing: 'clamp(1px, 0.4vw, 3px)',
                        ...
                        <span style={{
                          width: 'clamp(6px, 0.7vw, 8px)', height: 'clamp(6px, 0.7vw, 8px)',
                    ...
                    <div style={{
                      ...
                      <div style={{
                        fontFamily: FONT, fontSize: 'clamp(14px, 2vw, 22px)', letterSpacing: 'clamp(0.5px, 0.2vw, 1.5px)',
                        color: isH ? '#ffaa00' : TEXT2,
                      ...
                      <div style={{
                        fontFamily: FONT, fontSize: 'clamp(12px, 1.5vw, 18px)',
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 'clamp(6px, 1vw, 12px)' }}>
                        <span style={{ fontFamily: FONT, fontSize: 'clamp(11px, 1.5vw, 18px)', color: 'rgba(0,255,65,0.25)', letterSpacing: 'clamp(0.5px, 0.2vw, 1.5px)' }}>
                          {mod.duration}
                        </span>
                        <span style={{ fontFamily: FONT, fontSize: 'clamp(12px, 1.8vw, 20px)', color: isH ? '#ffaa00' : TEXT2, letterSpacing: 'clamp(0.5px, 0.2vw, 1.5px)' }}>
                          {isH ? '[EJECUTAR ▶]' : '[──────]'}
                        </span>
                      </div>
                    </div>
